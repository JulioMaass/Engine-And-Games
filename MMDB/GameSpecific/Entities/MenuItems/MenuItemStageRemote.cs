...
public class MenuItemStageRemote : Entity
{
    public MenuItemStageRemote()
    {
        AddStageMenuItemComponents();
        MenuItem.OnSelect = () =>
        {
            MenuManager.Clear();
            StageManager.CurrentStageName = "remoteStage";
            GameLoopManager.ResetGameLoop();
            StageEditor.TileMode.SetTilesetOfType(typeof(RemoteTileset));
        };
    }
}