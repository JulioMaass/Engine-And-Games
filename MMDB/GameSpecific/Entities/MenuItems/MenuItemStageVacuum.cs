...
public class MenuItemStageVacuum : Entity
{
    public MenuItemStageVacuum()
    {
        AddStageMenuItemComponents();
        MenuItem.OnSelect = () =>
        {
            MenuManager.Clear();
            StageManager.CurrentStageName = "vacuumStage";
            GameLoopManager.ResetGameLoop();
            StageEditor.TileMode.SetTilesetOfType(typeof(VacuumTileset));
        };
    }
}